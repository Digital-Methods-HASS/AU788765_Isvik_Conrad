...
Western_Europe %>% 
  ggplot(aes(x = Year, y = homicides, color= countries))+
  geom_line()+
  theme_classic()+
...
Western_Europe %>% 
  ...
  theme(text = element_text(size=18))
ggsave("homicides.png", width = 15, height = 8, dpi = 150)
...
The duration of a danish monarchs reign has clearly gone up while historical homicides rates clearly has gone down over time. 

```{r eval = TRUE}
# YOUR CODE HERE:
library(tidyverse)
kings <- read_csv2("data/kongerækken.csv")
...
kings <- kings %>%
  mutate(midyear = reign_end - (reign_end - reign_start)/2,
         duration = reign_end - reign_start)
library(ggplot2)
...
Based on the data visualisations the question about are we more civilized is a clear yes, yes we are. But what does civilized even mean? If it just means that we dont kill each other as much as we back in the time than yes again, we are way more civilized but we still kill each other from time to time which means we still havent totally learned our lesson. As we see in the visualisation above can we at least based on the western european countries conclude that we have become more civilized.
