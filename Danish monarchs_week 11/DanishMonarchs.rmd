...
separator:The columns is seperated by comma(,).
...
"spec_tbl_df" "tbl_df"      "tbl"         "data.frame", which means the dataset is saved as a tibble
...
```{r eval = TRUE}
# YOUR CODE
# changing from characters to numeric
library(tidyverse)
...
Longer_duration <- filter(kingsduration,duration>avg_duration)
view(Longer_duration)
select(Longer_duration,name,duration)
```
...
```{r eval = TRUE}
# YOUR CODE
top_duration <-arrange(kingsduration,desc(duration))
glimpse(top_duration)
mutate(top_duration,reign_days=duration*365+(duration%/%4))
